...
if (hay) {
  await p.evaluate(() => { const r=[...document.querySelectorAll('.fin-prod-row')].find(x=>/LEV024/.test(x.textContent)) || document.querySelector('.fin-prod-row'); r.click(); });
  ...
  const el = await p.$('.fpd');
  if (el) { await el.scrollIntoViewIfNeeded(); await p.waitForTimeout(400); await el.screenshot({ path: '_841_tramos.png' }); await b.close(); process.exit(0); }
  ...
  if (caja && caja.height > 40) await p.screenshot({ path: '_841_tramos.png', clip: caja });
  else await p.screenshot({ path: '_841_tramos.png' });
}
await b.close();
