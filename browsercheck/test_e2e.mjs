import fs from 'fs';
import pg from 'C:/Users/ISO/ProyectoMOS/node_modules/pg/lib/index.js';
...
try{
  ...
  console.log('token minteado OK (app claim server-side)');
  async function test(tipo,bloque){
    const body={model:'claude-sonnet-5',max_tokens:8192,system,messages:[{role:'user',content:[bloque,{type:'text',text:'Lee esta lista/tabla y extrae cada producto con su cantidad SOLICITADA (no el mín/máx/stock/precio). Devuelve solo el JSON indicado.'}]}]};
