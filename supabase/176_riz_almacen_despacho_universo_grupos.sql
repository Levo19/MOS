-- 176_riz_almacen_despacho_universo_grupos.sql
-- [RIZ · FIX UNIVERSO ALMACEN (despacho real) + campo `grupo` (ROTADO/PARADO) en TODAS las filas]
-- ════════════════════════════════════════════════════════════════════════════════════════════════════════════
-- CONTEXTO (decisión de compras — app de DINERO; lectura, no write-path, pero los números deciden plata):
--   La versión VIVA del panel es 173 (= 158 + pedidoEstado). En ámbito ALMACEN su universo sale de
--   me.zona_esperado(ALMACEN), que el cron riz-recompute-semanal (130) arma desde VENTAS de la estación
--   "Estacion 01" (ES004 → ALMACEN), NO del almacén físico. Resultado: productos que el almacén DESPACHA a
--   zonas pero que no se venden por esa estación NO aparecen (ej. PISTACHO CON SAL 100GR, WHPIOXAL100GR /
--   sku_base LEV1432: se despacha por SALIDA_ZONA, tiene stock 3, y NO estaba en los 403 del almacén).
--
-- ESTE ARCHIVO redefine me.zona_panel SOLO para corregir la rama ALMACEN + agregar el campo `grupo`:
--   ── PARTE A · ALMACEN (universo + objetivo desde DESPACHO real — reusa el enfoque INERTE de 137) ──
--     · Universo ALMACEN = (a) productos DESPACHADOS a zonas en las últimas 4 sem (wh.guia_detalle de wh.guias
--       tipo SALIDA_ZONA/SALIDA_JEFATURA, estado CERRADA/AUTOCERRADA, no anuladas, cant_recibida>0, ventana
--       rolling hoy-28..hoy) ∪ (b) productos con wh.stock.cantidad_disponible <> 0. Mapeo codBarra↔sku_base
--       vía mos.productos/equivalencias (mismo cb_sku que 173).
--     · OBJETIVO ALMACEN basado en VOLUMEN DE DESPACHO (no ventas de estación): pico semanal (máx diario por
--       semana ISO CERRADA, 4 sem) → esperado = ceil(pico_ULTIMA_sem × 1.2) [mismo colchón ×1.2 que 137/128].
--       tendencia/bcg por la MISMA matemática que me._riz_picos (regr_slope normalizada vs umbral 0.10 + mediana
--       de volumen del almacén). brecha = max(0, esperado − stockAlmacen). stockZona(ALMACEN)=stock de almacén.
--     · stockAlmacen del panel SIGUE saliendo de wh.stock (igual que 173). codigos[] de ALMACEN = desglose
--       base+equiv de wh.stock.
--   ── ZONAS (ZONA-01/02/…): SIN CAMBIOS de universo ni de números. Su universo sigue siendo
--      esp(me.zona_esperado de ventas) ∪ cod_arr(me.stock_zonas). Solo se les AGREGA el campo `grupo`.
--   ── PARTE A.3 · campo `grupo` en CADA fila (almacén Y zonas) ──
--     · 'ROTADO' = el producto rotó en 4 sem (ALMACEN: tuvo despacho>0 ; ZONA: tuvo venta / volumen>0).
--     · 'PARADO' = está en el universo por tener stock pero SIN rotación en 4 sem.
--     · Mutuamente excluyentes: si rotó → ROTADO aunque tenga stock; sin rotar pero con stock → PARADO.
--       (En zonas el "rotó" se deriva del volumen del esperado materializado, que es ventas 4 sem; en almacén
--        del despacho 4 sem. Para zonas sin esperado pero con stock → PARADO.)
--
-- PATRÓN intacto: security definer · set search_path='' · gate mos._claim_ok() · shape {ok,data} camelCase ·
--   lectura concatena || mos._frescura_sombra(). ADITIVO/backward-compatible: el shape data.items[] de 173 se
--   conserva tal cual (mismas claves: skuBase, descripcion, stockZona, esperada, brecha, stockNegativo,
--   stockAlmacen, tendencia, bcg, picos, volumen, unidad, esGranel, codigos, accionPerro, pedidoEstado,
--   vencimientoProximo, countLotes) + se AGREGA `grupo`. El wrapper mos.zona_panel (132) es pass-through → no
--   se toca. Money/inventario-safe: NO escribe nada (función STABLE de solo-lectura).
-- ════════════════════════════════════════════════════════════════════════════════════════════════════════════
...
declare
  ...
  -- ventana del UNIVERSO de despacho (rolling 4 sem): captura despachos recientes (incl. semana en curso) para
  -- que un producto recién despachado aparezca de inmediato.
  v_desde_uni  date := ((now() at time zone 'America/Lima')::date - 28);
  -- ventana del OBJETIVO (4 semanas ISO CERRADAS) — misma técnica que me._riz_picos (la semana en curso no cuenta).
...
begin
  ...
  -- ── DESPACHO del almacén (4 sem) por skuBase — SOLO se usa en rama ALMACEN ────────────────────────────────
  --    Universo (ventana rolling) + serie de picos semanales (ventana ISO cerrada) para el objetivo.
  desp_base as (
    ...
    where v_es_almacen
      ...
      and (g.fecha at time zone 'America/Lima')::date >= v_desde_uni
      and (g.fecha at time zone 'America/Lima')::date <= v_hoy
  ),
  -- universo despacho = cualquier sku con despacho en la ventana rolling 4 sem.
  desp_uni as (
