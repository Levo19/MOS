-- 587 · [Editor adhesivos · cero-GAS] get_flags() NO exponía 'adhesivosEdge', así que el frontend
-- (_mosAdhesivosEdge) lo leía OFF y CAÍA A GAS (muerto) al listar/guardar plantillas → editor
-- mostraba 'Sin plantillas guardadas'. MOS_ADHESIVOS_EDGE ya está en 1 en config; ahora get_flags
-- lo emite → el editor se queda 100% en Supabase (sin rastro de GAS). Regla get_flags-purga-gate.
