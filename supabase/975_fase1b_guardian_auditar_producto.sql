...
begin
  ...
  -- [GUARDIÁN F1] Auditar también es un ajuste (SET absoluto). Bloquear si el producto está en guía abierta:
  --   al cerrarla se sumaría su delta encima del conteo → doble conteo. ANTES del dedup (no consumir el slot).
  if coalesce((select valor from mos.config where clave='AJUSTE_BLOQUEA_GUIA_ABIERTA' limit 1),'1') = '1' then
    ...
    if v_gopen is not null then
      return jsonb_build_object('ok',false,'error','PRODUCTO_EN_GUIA_ABIERTA','guia',v_gopen,
        'mensaje','No puedes auditar: este producto está en una guía abierta ('||v_gopen||'). Ciérrala primero y vuelve a intentar.');
    end if;
  end if;
  ...
  v_result := case when v_diff = 0 then 'OK' else 'DIFERENCIA' end;
  ...
  return jsonb_build_object('ok',true,'dedup',false,'id_auditoria',v_idaud,'diferencia',v_diff,'resultado',v_result,'ajusto',v_diff <> 0);
end;
$function$
