-- 586 · [Envasado · identidad del operador] Fix + higiene, óptimo (1 punto server-side, reusa _norm_nom):
--  (1) wh.registrar_envasado normaliza `usuario` (trim + colapsa espacios, preserva mayúsculas) en el
--      declare → aplica a envasados + guías + movimientos a la vez. Origen del 'Jesus ' = el front
--      concatena nombre+' '+apellido y sin apellido queda con espacio (se arregla también en el front).
--  (2) Guard "operador fantasma": rechaza registrar envasado con usuario='' o _norm_nom='operador'
--      (WH_CONFIG.usuario default sin login) → OPERADOR_REQUERIDO. Antes se guardaba a un genérico que
--      NO paga a nadie (el recompute matchea por _norm_nom y 'operador' no es ningún personal) y se ve aparte.
--  (3) Histórico: normaliza usuario en TODAS las filas (fusiona variantes: 'Jesus '→'Jesus',
--      'Oswaldbit  Esaa '→'Oswaldbit Esaa') y reasigna el registro puntual mal-atribuido a 'Jesus'
--      (confirmado por el dueño). El trigger de recompute re-acredita a Jesús si el día no está sellado;
--      el guard_sellado NO bloquea (solo protege cambios de unidades/colaborador/estado, no de usuario).
-- Regex POSIX [[:space:]]+ (NO '\s' — con esta conexión se interpretaba como 's' literal y borraba letras).
...
-- (3a) higiene: normalizar usuario en todo el histórico (trim + colapsar espacios)
update wh.envasados
   set usuario = btrim(regexp_replace(usuario,'[[:space:]]+',' ','g'))
 where usuario is distinct from btrim(regexp_replace(usuario,'[[:space:]]+',' ','g'));

-- (3b) reasignar el envasado mal-atribuido ('operador' → 'Jesus'), confirmado por el dueño
update wh.envasados set usuario = 'Jesus'
 where id_envasado = 'ENV_L1785170918520v867hux' and mos._norm_nom(usuario) = 'operador';
