-- 327_cerrar_caja_guard_idcaja.sql
-- [FIX money-critico] me.cerrar_caja: guard id_caja = v_id en la anula de POR_COBRAR.
-- Antes, ids_anular con IDs de OTRA caja/zona podia anular sus POR_COBRAR (bug: ventas_hoy_zona
-- filtra por prefijo de correlativo, no por caja). Ahora NUNCA se anula un ticket de otra caja,
-- aunque el cliente (version vieja cacheada) mande IDs cross-zona. Defensa server-side.
  ...
  with anuladas as (
    update me.ventas
     ...
     where upper(forma_pago) = 'POR_COBRAR'
       and id_caja = v_id   -- [FIX money-critico 2026-07-03] NUNCA anular POR_COBRAR de OTRA caja (guard server-side)
       and ( v_ids_anular is null or id_venta = any(v_ids_anular) )
    returning id_venta
  )
