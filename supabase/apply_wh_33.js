...
const sql = fs.readFileSync(__dirname + '/33_wh_actualizar_preingreso.sql', 'utf8');
(async () => {
  await c.connect();
  try {
    await c.query('begin'); await c.query(sql);
    const fn = await c.query(`select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='wh' and p.proname='actualizar_preingreso'`);
    const flag = await c.query(`select valor from mos.config where clave='WH_ACTUALIZAR_PREINGRESO_DIRECTO' limit 1`);
    const inert = !flag.rows.length || flag.rows[0].valor !== '1';
    console.log('wh.actualizar_preingreso existe:', fn.rowCount === 1, '| flag:', flag.rows[0] ? flag.rows[0].valor : '(seed)', '| inerte:', inert);
