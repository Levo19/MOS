-- ════════════════════════════════════════════════════════════════════
-- 570 · FIX regresión de 569: al reproducir a mano _voucher_guia y get_reporte se perdió
-- el fallback `pn.descripcion` en el coalesce del nombre → un PRODUCTO NUEVO (registrado en
-- wh.producto_nuevo con el nombre que tipeó el operador) mostraba el CÓDIGO CRUDO en el
-- voucher-imagen y en el reporte público. Se restaura: coalesce(pr, eqp, pn, código).
-- (operacion_detalle NO usa pn → no afectada.) Todo lo demás idéntico a 569.
-- ════════════════════════════════════════════════════════════════════
...
grant execute on function wh.get_reporte(jsonb) to anon, authenticated, service_role;
