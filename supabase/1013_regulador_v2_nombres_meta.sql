-- ============================================================================
-- 1013_regulador_v2_nombres_meta.sql — Regulador v2 (feedback del dueño 02-sep)
-- ----------------------------------------------------------------------------
-- 1) P1 negativos: los códigos sin nombre (equivalentes, id_producto, presentaciones)
--    ahora resuelven al producto CANÓNICO: codigo_barra → equivalencia→canónico →
--    id_producto → sku_base. "Lo mismo de siempre: no consideraba equivalentes."
-- 2) P5 auditorías: la meta es 30 conteos POR OPERADOR POR DÍA (mos.config
--    evalMetaAuditorias, default 30). Por operador: HOY x/meta, AYER, y DEUDA de días
--    anteriores = Σ max(0, meta − conteos) sobre sus días PRESENTES (liquidaciones_dia;
--    si el nombre no matchea asistencia, sobre sus días con conteos). Igual en almacén
--    (wh.auditorias por usuario). Reemplaza mos.regulador_reporte completo.
-- ============================================================================
...
begin
  ...
  foreach v_z in array v_zonas loop
    -- ── P1 · negativos (nombre canónico: código → equivalencia → id → sku) ──
    -- [v2] AGRUPADO POR PRODUCTO (canónico): la vista de la zona cuenta TARJETAS (equivalentes
    -- plegados en su canónico), no filas por código. `n` = productos (cuadra con el banner
    -- "⚠ N con stock en negativo"); `codigos` = filas crudas (la cifra vieja).
    if v_z = 'ALMACEN' then
      with neg as (
        select s.cod_producto cod, s.cantidad_disponible cant,
               coalesce(pr.sku_base, eq.sku_base,
                        (select p3.sku_base from mos.productos p3 where p3.id_producto = s.cod_producto limit 1),
                        s.cod_producto) sku
          from wh.stock s
      ...
      grp as (
        select sku, sum(cant) cant, count(*) codigos, min(cod) cod,
               exists (select 1 from mos.productos pa
                        where (pa.sku_base = neg.sku or btrim(pa.codigo_barra) = btrim(neg.sku))
                          and coalesce(pa.estado::text,'1') <> '0') activo
          from neg group by sku)
      select jsonb_build_object('n', count(*), 'codigos', coalesce(sum(g.codigos),0),
        'activos', count(*) filter (where g.activo),
        'ocultos', count(*) filter (where not g.activo),
        'total', coalesce(round(sum(g.cant)::numeric,2),0),
        'items', coalesce(jsonb_agg(jsonb_build_object('cod', g.cod, 'cant', round(g.cant::numeric,2),
                   'activo', g.activo,
                   'nombre', coalesce(
                     ...
                     (select p5.descripcion from mos.productos p5 where btrim(p5.codigo_barra) = btrim(g.sku) limit 1),
                     g.cod)) order by g.cant asc) filter (where g.rn <= 15), '[]'::jsonb))
      into v_neg
      from (select *, row_number() over (order by cant asc) rn from grp) g;
    else
      with neg as (
        select s.cod_barras cod, s.cantidad cant,
               coalesce(pr.sku_base, eq.sku_base,
                        (select p3.sku_base from mos.productos p3 where p3.id_producto = s.cod_barras limit 1),
                        s.cod_barras) sku
