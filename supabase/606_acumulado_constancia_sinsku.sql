-- 606_acumulado_constancia_sinsku.sql — [WH · "NO SE ENTIENDE" TAMBIÉN EN EL ACUMULADO]
-- Pedido del dueño (2026-08-01): cuando un producto de una lista NO matchea (la IA no lo
-- identificó → sin código), debe guardarse TAMBIÉN en el acumulado de la zona con un TAG
-- distinto ("no se entendió"), como constancia del pedido — SIN sumar deuda jamás.
-- ANTES: consolidar_pickup_zona los SALTEABA (continue en skuBase='') → la constancia solo
-- vivía en la lista sombra; al absorberse, el "no entendido" desaparecía del acumulado.
-- AHORA: viajan como items {skuBase:'', sinSku:true, nombre, solicitado:0, despachado:0,
-- constancia:<uds pedidas>} con clave 'SINSKU::<nombre>' (se suman entre listas por nombre).
-- solicitado=0 ⇒ NINGÚN cálculo de deuda/uds los cuenta (piso garantizado por diseño).
-- El checklist de WH les da su sección "❓ No se entiende" (2.13.523), igual que en la sombra.
...
declare
  ...
  v_it        jsonb;
  v_sku       text;
  v_key       text;      -- [606] clave de constancia SINSKU::<nombre>
  v_c         numeric;   -- [606] uds de constancia
  v_pend      numeric;
  v_sol_add   numeric;
  v_desp_add  numeric;
  v_cand      record;
...
begin
  ...
  if v_existing is not null and jsonb_typeof(v_existing) = 'array' then
    for v_it in select * from jsonb_array_elements(v_existing) loop
      ...
      if v_sku = '' then
        -- [606] constancia "no se entendió": se ARRASTRA (antes se perdía), jamás suma deuda
        if coalesce((v_it->>'sinSku')::boolean, false) then
          ...
          if v_c > 0 then
            ...
            v_map := jsonb_set(v_map, array[v_key], jsonb_build_object(
              ...
              'solicitado', 0, 'despachado', 0, 'constancia', v_c), true);
          end if;
        end if;
        continue;
      end if;
  ...
  loop
    if jsonb_typeof(v_cand.items) = 'array' then
      for v_it in select * from jsonb_array_elements(v_cand.items) loop
        v_sku := coalesce(v_it->>'skuBase', '');
        if v_sku = '' then
          -- [606] constancia "no se entendió" de la lista absorbida → al acumulado, sumando por nombre
          if coalesce((v_it->>'sinSku')::boolean, false) then
            v_c := wh._num(coalesce(v_it->>'constancia', v_it->>'solicitado', '0'));
            if v_c > 0 then
              v_key := 'SINSKU::' || coalesce(nullif(btrim(v_it->>'nombre'),''), 'SIN NOMBRE');
