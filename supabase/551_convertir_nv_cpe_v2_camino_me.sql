-- ════════════════════════════════════════════════════════════════════
-- 551 — FIX CRÍTICO pre-go-live: me.convertir_nv_cpe estaba ROTA.
-- La v1 emitía vía fac.emitir_cpe (capa INERTE, FAC_CPE_DIRECTO=0) y el
-- fallback GAS fue eliminado (cero-GAS 2026-07-14) → TODA conversión
-- terminaba en "Sin conexión con el servidor" (0 conversiones reales
-- en el historial). Además, dejarla en fac chocaría correlativos cuando
-- fac se prenda para MOS-VIP (fac.series ≠ me.correlativos).
--
-- v2 = el MISMO camino del POS (una sola numeración por serie):
--   1) valida clave admin + reglas SUNAT de cliente (server-side)
--   2) serie por ZONA de la NV (mos.series_documentales, tiebreaker id_serie)
--   3) correlativo me.siguiente_correlativo(serie, idem_key) — idempotente
--   4) anula la NV (ANULADO_CONVERSION) + crea la venta CPE PENDIENTE
--      con el detalle copiado (misma tx, atómico)
--   5) devuelve ventaBase para que el FRONTEND emita por el Edge
--      emitir-cpe (token en secret) + set_cpe_nf — igual que el POS.
--   Si el Edge falla → la venta queda PENDIENTE y la RECONCILIACIÓN
--   horaria la re-emite sola (misma red de seguridad del POS).
-- ════════════════════════════════════════════════════════════════════
drop function if exists me.convertir_nv_cpe(jsonb);
create function me.convertir_nv_cpe(p jsonb default '{}'::jsonb)
...
begin
  ...
  select serie into v_serie from mos.series_documentales
   where activo and upper(tipo_documento) = v_tipo
     and ( coalesce(v_nv.zona_id,'') = '' or id_zona = v_nv.zona_id )
   order by id_serie asc limit 1;
  if v_serie is null or btrim(v_serie) = '' then
    return jsonb_build_object('ok',false,'error','SERIE_REQUERIDA: la zona '||coalesce(v_nv.zona_id,'?')||' no tiene serie '||v_tipo||' activa en MOS config');
  end if;
