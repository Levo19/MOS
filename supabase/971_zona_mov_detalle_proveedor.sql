-- [971] (deriva de 968) + proveedor/numero_documento en guía de almacén.
...
begin
  ...
  if v_fte = 'venta' then
    ...
    select to_jsonb(h) into v_head from (
      select g.id_guia, coalesce(g.tipo,'') tipo,
             to_char(g.fecha at time zone 'America/Lima','YYYY-MM-DD HH24:MI') fecha,
             coalesce(g.usuario,'') vendedor, coalesce(g.id_zona,'') zona, coalesce(pv.nombre,'') proveedor, coalesce(g.numero_documento,'') doc,
             ''::text destino, coalesce(g.estado,'') estado, coalesce(g.comentario,'') obs
        from wh.guias g left join mos.proveedores pv on pv.id_proveedor = g.id_proveedor where g.id_guia = v_id) h;
...
select '971 zona_mov_detalle + proveedor listo' as ok;
