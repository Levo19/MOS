-- ════════════════════════════════════════════════════════════════════════════
-- 239 · get_flags += adhesivosEdge (flip de #5: Editor de Adhesivos → Supabase)
-- ════════════════════════════════════════════════════════════════════════════
-- Agrega `adhesivosEdge` a mos.get_flags (lee MOS_ADHESIVOS_EDGE), preservando TODAS
-- las keys (incl. sunatEdge del 236). El front (api.js _mosAdhesivosEdge) lo consume →
-- con '1' el editor lee/guarda/imprime por Supabase (RPCs mos.adhesivo_* + Edge
-- print-adhesivo-plantilla); con GAS de red de seguridad. Backfill dio 0 plantillas
-- (no había ninguna) → flip seguro. KILL-SWITCH: MOS_ADHESIVOS_EDGE='0'.
-- ════════════════════════════════════════════════════════════════════════════

insert into mos.config (clave, valor, descripcion) values
  ('MOS_ADHESIVOS_EDGE', '1', '#5: el Editor de Adhesivos usa Supabase (RPCs mos.adhesivo_* + Edge print-adhesivo-plantilla) en vez de GAS. 0=GAS, 1=Supabase (con GAS de red de seguridad).')
...
returns jsonb language sql stable security definer set search_path = '' as $function$
  ...
  select jsonb_build_object(
    ...
    -- ── [#5 Editor de Adhesivos] CRUD por RPCs + impresión por Edge ──
