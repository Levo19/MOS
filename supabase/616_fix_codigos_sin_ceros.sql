-- 616 · Los códigos de barra son TEXTO: recuperar los ceros a la izquierda perdidos.
--
-- Luis (03/08/2026): "¿por qué veo los códigos de barra sin ceros? si el código de barra
-- debe ser todo en formato texto... donde sea que se use deben respetar eso".
--
-- DIAGNÓSTICO: todas las columnas de código en las 3 apps son `text` (la BD está bien).
-- El daño está en los DATOS de `mos.proveedores_productos.codigo_barra`: guarda '247'
-- donde el catálogo tiene '00247' — alguien lo pasó por un número y volvió a texto.
-- Casos reales: 247→00247, 574→00574, 27→00027, 1119→01119, y hasta un código de 12
-- dígitos: 18850892005 (le falta el 0 inicial de 018850892005).
--
-- CONSECUENCIAS que esto causaba:
--   · La card mostraba "247" en vez de "00247" (lo que Luis vio).
--   · El cruce con el catálogo fallaba → el producto se quedaba sin stock por ubicación
--     y caía al render viejo (por eso CAMARON DESHIDRATADO no tenía los cuadros nuevos).
--   · Fue la causa del hallazgo H1 (195 de 746 productos sin datos).
--
-- QUÉ HACE: repara el código SOLO cuando el sku_base resuelve a UN producto del catálogo
-- y la única diferencia son ceros a la izquierda. No inventa nada: es el mismo código.
-- Idempotente (la 2ª corrida no encuentra nada que cambiar).

do $$
declare v_n int;
begin
  with objetivo as (
    select pp.id_pp, pr.codigo_barra as cod_bueno
      from mos.proveedores_productos pp
      join mos.productos pr
        on coalesce(nullif(btrim(pr.sku_base),''), pr.id_producto) = btrim(pp.sku_base)
     where nullif(btrim(pp.codigo_barra),'') is not null
       and btrim(pp.codigo_barra) <> btrim(pr.codigo_barra)
       -- mismo código salvo ceros a la izquierda (y ambos numéricos: sin riesgo alfanumérico)
       and ltrim(btrim(pp.codigo_barra), '0') = ltrim(btrim(pr.codigo_barra), '0')
       and btrim(pp.codigo_barra) ~ '^[0-9]+$'
       and btrim(pr.codigo_barra) ~ '^[0-9]+$'
       -- el sku debe resolver a UN solo producto (si no, no tocar)
       and (select count(*) from mos.productos p2
             where coalesce(nullif(btrim(p2.sku_base),''), p2.id_producto) = btrim(pp.sku_base)
               and p2.tipo_producto::text = 'CANONICO') = 1
       and pr.tipo_producto::text = 'CANONICO'
  )
  update mos.proveedores_productos pp
     set codigo_barra = o.cod_bueno
    from objetivo o
   where pp.id_pp = o.id_pp;
  get diagnostics v_n = row_count;
  raise notice '616: códigos de proveedor reparados (ceros a la izquierda) = %', v_n;
end $$;

-- La RPC devuelve AMBOS códigos: el del catálogo (canónico, para mostrar) y el que el
-- proveedor tiene guardado (para que el front pueda cruzar por cualquiera de los dos,
-- aunque algún dato vuelva a llegar mutilado desde otra vía).
