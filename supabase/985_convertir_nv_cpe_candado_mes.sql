-- [985] Convertir NV -> CPE: CANDADO DE MES. Solo se puede convertir un ticket dentro del MISMO mes
--  calendario (hora Peru). Un ticket de un mes anterior queda BLOQUEADO (emitirlo hoy declararia su IGV
--  en el periodo actual). Es la unica diferencia vs la version 585; el resto queda IDENTICO (money/stock-safe).
--  Off-switch: mos.config CPE_CONVERT_MES_ESTRICTO=0.
...
begin
  ...
  if coalesce((select valor from mos.config where clave='CPE_CONVERT_MES_ESTRICTO' limit 1),'1') <> '0'
     and date_trunc('month', (v_nv.fecha at time zone 'America/Lima'))
         < date_trunc('month', (now() at time zone 'America/Lima')) then
    return jsonb_build_object('ok',false,'error','MES_ANTERIOR',
      'mensaje','Este ticket es de un mes anterior ('||to_char(v_nv.fecha at time zone 'America/Lima','DD/MM/YYYY')||'). Solo se convierte dentro del mismo mes calendario, para no declarar IGV de un mes en otro.',
      'fechaTicket', to_char(v_nv.fecha at time zone 'America/Lima','YYYY-MM-DD'));
  end if;
...
select '985 convertir_nv_cpe candado de mes listo' as ok;
