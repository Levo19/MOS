-- 597_catalogo_envase_insumo.sql — [CATÁLOGO · ENVASES DE DERIVADOS + INSUMOS]
-- Pedido del dueño (2026-07-31): los derivados consumen un envase (celofán) al envasarse.
-- (1) mos.productos.envase_sku  text  → sku_base del INSUMO que usa el derivado.
--     null = falta elegir (chip punteado en catálogo) · 'SIN_ENVASE' = decisión explícita de no llevar.
--     Se guarda el sku_base (NO codigo_barra ni nombre: esos pueden cambiar, el sku_base jamás).
-- (2) mos.productos.es_insumo boolean → toggle "Insumo de envasado" (celofanes hoy; cualquier
--     producto, canónico incluido, puede marcarse). El modal +Derivado lista SOLO es_insumo=true.
-- (3) crear_producto / actualizar_producto aceptan envaseSku / esInsumo (patch parcial, vaciables).
-- (4) Seed: todos los CELOFAN → es_insumo=true. (El seed por-derivado va en 598, generado del
--     match verificado por el dueño en el HTML de revisión.)
-- NOTA sync: migrarCatalogoCompartido es upsert-only de columnas del spec GAS → NO toca estas columnas.

-- (1)(2) columnas
alter table mos.productos add column if not exists envase_sku text;
alter table mos.productos add column if not exists es_insumo boolean not null default false;

comment on column mos.productos.envase_sku is
  'sku_base del insumo (envase) que consume este DERIVADO al envasarse. null=falta elegir · SIN_ENVASE=no lleva (decisión explícita). Solo aplica a derivados.';
comment on column mos.productos.es_insumo is
  'true = insumo de envasado (celofán/descartable): aparece en el selector de envase del modal +Derivado.';

-- (4) seed celofanes (34 al 2026-07-31; ilike cubre altas con el mismo nombre)
update mos.productos set es_insumo = true, updated_at = now()
 where descripcion ilike 'CELOFAN%' and coalesce(es_insumo,false) = false;

-- (3a) crear_producto + envase_sku/es_insumo
...
begin
  ...
  -- [597] + envase_sku (sku_base del insumo del derivado) + es_insumo (toggle insumo de envasado)
...
-- (3b) actualizar_producto + envase_sku/es_insumo (patch parcial: solo si la clave viene; envaseSku vaciable)
