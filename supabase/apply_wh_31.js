// Aplica 31_wh_registrar_merma.sql al proyecto MOS, atómico + verificado. RPC INERTE (flag '0') → seguro.
...
const sql = fs.readFileSync(__dirname + '/31_wh_registrar_merma.sql', 'utf8');
(async () => {
  await c.connect();
  try {
    await c.query('begin'); await c.query(sql);
    const fn = await c.query(`select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='wh' and p.proname='registrar_merma'`);
    const flag = await c.query(`select valor from mos.config where clave='WH_REGISTRAR_MERMA_DIRECTO' limit 1`);
    const inert = !flag.rows.length || flag.rows[0].valor !== '1';
    console.log('wh.registrar_merma existe:', fn.rowCount === 1, '| flag:', flag.rows[0] ? flag.rows[0].valor : '(seed)', '| inerte:', inert);
