-- ════════════════════════════════════════════════════════════════════
-- 567 · Resolver CÓDIGOS EQUIVALENTES en el voucher y en el reporte público.
-- Un código puede ser un equivalente (mos.equivalencias.codigo_barra → sku_base);
-- su nombre real está en mos.productos vía sku_base (canónico: codigo_producto_base
-- is null). Antes solo se miraba productos.codigo_barra directo → equivalentes (ej.
-- MAGGI) salían "sin identificar". Ahora se resuelven y NO se marcan.
-- Afecta: wh._voucher_guia (SQL 566) y wh.get_reporte (SQL 410).
-- ════════════════════════════════════════════════════════════════════
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'esProductoNuevo', (pr.descripcion is null and eq.sku_base is null and (pn.descripcion is not null or d.cod_producto like 'NLEV%')),
    'esSinIdentificar',(pr.descripcion is null and eq.sku_base is null and pn.descripcion is null and d.cod_producto not like 'NLEV%')
  ) order by d.linea), '[]'::jsonb) into v_det
  from wh.guia_detalle d
  left join mos.productos pr on pr.codigo_barra = d.cod_producto
  left join lateral (select sku_base from mos.equivalencias e where e.codigo_barra = d.cod_producto and e.activo is true limit 1) eq on true
  left join lateral (select descripcion from mos.productos px where px.sku_base = eq.sku_base order by (px.codigo_producto_base is null) desc, px.id_producto limit 1) eqp on true
...
-- ── get_reporte: misma resolución de equivalentes en el detalle de la guía ──
...
begin
  ...
  if v_tipo = 'guia' then
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'esProductoNuevo',  (pr.descripcion is null and eq.sku_base is null and (pn.descripcion is not null or d.cod_producto like 'NLEV%')),
      'esIncompleto',     (pr.descripcion is null and eq.sku_base is null and pn.descripcion is null and d.cod_producto not like 'NLEV%'),
