-- ============================================================================
-- 314_me_cobrar_credito_directo.sql — Cobro DIRECTO de crédito (admin) cero-GAS
-- ----------------------------------------------------------------------------
-- Replica gas EditarVenta.gs::cobrarCreditoConExtra (el primitivo de cobro directo del
-- panel MOS: el admin cobra un crédito eligiendo la caja receptora, SIN cobro asignado
-- previo). Distinto de me.confirmar_cobro (que es por idCobro, flujo del cajero).
--   1) valida venta pendiente (CREDITO/POR_COBRAR, no ANULADO) + caja receptora ABIERTA;
--   2) crea el/los movimiento(s) INGRESO/INGRESO_VIRTUAL/MIXTO en la caja receptora
--      (mismas filas que el GAS → el cierre las cuadra idéntico);
--   3) flip me.ventas.forma_pago al método (raw, paridad Sheet setValue).
-- MEJORA money-safe sobre GAS: si existía un cobro ASIGNADO para esa venta, lo marca
--   COBRADO (el GAS lo dejaba colgado y lo reconciliaba un cron → riesgo de cobro fantasma).
-- Atómico (advisory-lock por venta), idempotente (idExtra determinista EX-DIR-<idVenta>).
-- Gate jwt_app MOS/mosExpress + ME_COBRO_DIRECTO='1' (ya EN VIVO).
-- ============================================================================
...
$fn$;
revoke all on function me.cobrar_credito_directo(jsonb) from public;
grant execute on function me.cobrar_credito_directo(jsonb) to authenticated, service_role;
