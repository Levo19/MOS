-- ============================================================
-- 425 · mos.analitica_grupo — analítica FUSIONADA almacén(WH) + zonas(ME) por grupo canónico
-- ============================================================
...
-- Nota: NO es `stable` porque usa temp table (grupo extendido reutilizado por 6 CTEs);
-- volatile no afecta a PostgREST (una tx por RPC, on commit drop limpia).
...
begin
  ...
  -- ── grupo extendido + factor a canónico por miembro ─────────────
  create temp table if not exists _ag_grupo (
    cb text primary key, tipo text, nombre text, f_canon numeric
  ) on commit drop;
  delete from _ag_grupo;

  insert into _ag_grupo (cb, tipo, nombre, f_canon)
  -- canónico + presentaciones del grupo raíz
  select upper(btrim(pr.codigo_barra)),
         case when coalesce(nullif(pr.factor_conversion,0),1) = 1 then 'granel' else 'presentacion' end,
         pr.descripcion,
         coalesce(nullif(pr.factor_conversion,0),1)
  from mos.productos pr
  where (pr.sku_base = v_sku or pr.id_producto = v_sku)
    and coalesce(nullif(btrim(pr.codigo_producto_base),''),'') = ''
    and coalesce(btrim(pr.codigo_barra),'') <> ''
  on conflict (cb) do nothing;

  -- equivalencias del grupo raíz (factor del canónico = 1)
  insert into _ag_grupo (cb, tipo, nombre, f_canon)
  select upper(btrim(e.codigo_barra)), 'equivalente', coalesce(e.descripcion,'equivalente'), 1
  from mos.equivalencias e
  where e.sku_base = v_sku and e.activo and coalesce(btrim(e.codigo_barra),'') <> ''
  on conflict (cb) do nothing;

  -- derivados del granel (codigo_producto_base = skuBase | idProducto del canónico)
  insert into _ag_grupo (cb, tipo, nombre, f_canon)
  select upper(btrim(d.codigo_barra)), 'derivado', d.descripcion,
         coalesce(nullif(d.factor_conversion_base,0), 0)
  from mos.productos d
  ...
  -- presentaciones y equivalencias DE los derivados (factor = pack × porción)
  insert into _ag_grupo (cb, tipo, nombre, f_canon)
  select upper(btrim(pp.codigo_barra)), 'pres_derivado', pp.descripcion,
         coalesce(nullif(pp.factor_conversion,0),1) * coalesce(nullif(d.factor_conversion_base,0),0)
  from mos.productos d
  ...
  -- alcance opcional (chips del panel: solo canónico / solo un satélite)
  if v_alcance is not null then
    delete from _ag_grupo where cb <> all(v_alcance);
  end if;
