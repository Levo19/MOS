-- 531_analitica_pos.sql — ANALÍTICA DE PRODUCTO EN EL POS (proyecto del dueño 2026-07-19).
-- Click derecho (PC) / mantener presionado (táctil) sobre un producto del catálogo ME abre
-- su card de analítica: el producto PADRE (canónico del grupo), ventas de la ZONA día a día
-- de las 4 semanas ISO completas ANTERIORES a la actual, lo vendido de ESTA semana, el
-- PRONÓSTICO día a día (promedio del mismo día de semana en las 4 previas), la TENDENCIA con
-- la MISMA lógica de MOS/zona (picos semanales + regr_slope/media, umbral 0.10) y el stock
-- de almacén (tiempo real) + stock de zona. Todo en UNA llamada.
...
declare
  ...
  v_lun   date := date_trunc('week', (now() at time zone 'America/Lima'))::date;  -- lunes de ESTA semana
  v_desde date;
...
begin
  ...
  -- ── producto → grupo (sku_base) → PADRE canónico (factor 1, sin código base) ──
  ...
  with grupo as (
    -- todos los códigos del grupo (canónico + presentaciones ×factor + equivalentes ×1)
  ...
  ventas_dia as (
    select (v.fecha at time zone 'America/Lima')::date as dia,
  ...
  -- 28 días previos (4 semanas ISO completas), con ceros
  hist as (
  ...
  -- pronóstico por día de semana = promedio del mismo isodow en las 4 semanas (ceil)
  pron as (
    select dow, ceil(avg(u))::numeric as pron from hist group by dow
  ),
  -- tendencia: MISMA lógica MOS/zona (picos semanales, pendiente/media, umbral 0.10)
  sem as (
  ...
  -- semana ACTUAL: real día a día (null en días futuros) + pronóstico
  actual as (
