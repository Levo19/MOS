-- 787 · ME: editar_guia_lineas v2 — devuelve lineasAgregadas [{linea,codBarras,cantidad}]
-- para que el editor optimista del front pueda seguir editando lo recién agregado.
-- (v1 = SQL 785; misma lógica de deltas idempotentes, solo cambia el retorno.)
...
begin
  ...
  loop
    ...
    v_agregadas := v_agregadas || jsonb_build_object('linea', v_max, 'codBarras', v_cb, 'cantidad', v_nueva);
    v_ag := v_ag + 1;
  end loop;
  ...
  return jsonb_build_object('ok', true, 'idGuia', v_id, 'estado', v_g.estado,
    'editadas', v_ed, 'eliminadas', v_el, 'agregadas', v_ag, 'lineasAgregadas', v_agregadas);
