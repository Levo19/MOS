-- ════════════════════════════════════════════════════════════════════════════════════════════════════════════
-- 197_riz_ajuste_todos_los_codigos_left_join.sql — AJUSTE muestra TODOS los códigos del grupo (canónico +
--   equivalentes activos), incluido el que está en 0 o SIN fila en la tabla de stock. Zona/RIZ (MOS).
-- App de DINERO en PROD · SOLO LECTURA (no escribe stock/dinero/sync). Aditivo: no cambia el shape JSON ni el
-- resto del panel (brechas, BCG, tendencias, universo, filtros, etc.).
-- ────────────────────────────────────────────────────────────────────────────────────────────────────────────
-- PROBLEMA
--   El array `codigos[]` (que el front despliega en el ajuste por código) se construía con un JOIN INTERNO de
--   los códigos del grupo (cb_sku) contra la tabla de stock:
--     · ZONA    → JOIN me.stock_zonas   (CTE stock_cod)
--     · ALMACÉN → JOIN wh.stock         (CTE stock_alm_cod)
--   Un código del grupo SIN fila en la tabla de stock quedaba FUERA del JOIN → NO se desplegaba en el ajuste,
--   aunque el historial (kardex) sí lo mostraba. Inconsistente y confuso.
--   Caso real medido: COCINERO LEV051 (ALMACÉN) tiene 3 códigos — canónico 7750243068048 + equivalentes activos
--   7750243079310 y 7750243077958. wh.stock sólo tiene fila para 068048 (=0) y 079310 (=0); 077958 NO tiene fila
--   → el ajuste sólo mostraba 2 de los 3 códigos (077958 invisible).
--
-- FIX (este archivo)
--   `codigos[]` = TODOS los códigos del grupo (cb_sku: canónico — EXCLUYE presentaciones, SQL 192 — ∪ equivalentes
--   activos), con LEFT JOIN a la tabla de stock correspondiente. Cada código sale con su stock (0 si no hay fila,
--   nunca se omite). Aplica a ZONA (me.stock_zonas) y ALMACÉN (wh.stock).
--     · ZONA  (stock_cod):     el filtro de zona se mueve a la condición del LEFT JOIN (NO al WHERE) para no
--                              degradar el LEFT JOIN a INNER. Se agrega DISTINCT por código antes de sumar (un
--                              código puede tener filas en varias zona_aliases; sólo cuentan las de las aliases).
--     · ALMACÉN (stock_alm_cod): LEFT JOIN wh.stock; sum(coalesce(...,0)) y count → 0/0 cuando no hay fila.
--
-- 🔴 GARANTÍA CRÍTICA (money / plata invisible)
--   NINGÚN código que TENGA stock (cantidad ≠ 0) puede quedar oculto del ajuste. Antes el riesgo era al revés
--   (omitir códigos por falta de fila); como ahora se ancla en cb_sku (TODOS los códigos del grupo) y se hace
--   LEFT JOIN, el conjunto de códigos emitidos es un SUPER-conjunto del anterior → es IMPOSIBLE que un código con
--   stock que antes se veía ahora desaparezca. La suma del grupo (stock_zona / stock_almacen) NO cambia: los
--   códigos nuevos aportan 0 (no tenían fila) → totales idénticos a antes.
--
--   Las CTEs de stock alimentan SÓLO el array `codigos[]`/`codigos_almacen[]` y la suma del grupo (stock_zona /
--   stock_almacen). El "universo" de almacén (sku_stock_alm, que decide qué PRODUCTOS aparecen) sigue exigiendo
--   Σ stock ≠ 0 a nivel SKU — sin cambios: esto NO oculta códigos dentro de un producto ya mostrado.
--
-- IDEMPOTENTE (create or replace). Regenera me.zona_panel completa (= versión live SQL 192) cambiando SÓLO las
-- dos CTEs de stock (stock_cod, stock_alm_cod) a LEFT JOIN. mos.zona_panel es un wrapper de me.zona_panel (sin
-- cambios). No toca firmas públicas, flags, sync ni dinero.
-- ════════════════════════════════════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  --        GARANTÍA money: ningún código con stock ≠ 0 puede faltar (cb_sku = super-conjunto; suma del grupo igual).
  stock_cod as (
