...
const sql = fs.readFileSync(__dirname + '/32_wh_crear_preingreso.sql', 'utf8');
(async () => {
  await c.connect();
  try {
    await c.query('begin'); await c.query(sql);
    const fn = await c.query(`select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='wh' and p.proname='crear_preingreso'`);
    const flag = await c.query(`select valor from mos.config where clave='WH_CREAR_PREINGRESO_DIRECTO' limit 1`);
    const inert = !flag.rows.length || flag.rows[0].valor !== '1';
    console.log('wh.crear_preingreso existe:', fn.rowCount === 1, '| flag:', flag.rows[0] ? flag.rows[0].valor : '(seed)', '| inerte:', inert);
