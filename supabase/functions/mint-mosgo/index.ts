// Edge Function `mint-mosgo` — EMITE el JWT Supabase (HS256, claim app='mosGo') que la PWA de MOS (master) usará,
...
//      estado='ACTIVO' y app ∈ {null, '', 'mosGo'}. FAIL-CLOSED: cualquier error/no-match → no token.
//   2) app='mosGo' y exp están HARDCODEADOS server-side; el body solo aporta el deviceId (= sub). El cliente no
...
const APP = 'mosGo';      // claim app HARDCODEADO server-side (NO se toma del body).
