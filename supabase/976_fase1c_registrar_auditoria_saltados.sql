...
declare
  ...
  v_n int := 0;
  v_saltados jsonb := '[]'::jsonb;   -- [F1] productos NO contados por estar en guía abierta
begin
  ...
  for v_it in select * from jsonb_array_elements(v_items) loop
    ...
    -- [GUARDIÁN F1] si el producto está en guía abierta, el ajuste se rechaza → NO registrar la auditoría
    --   (evita "conté 30" sin haber tocado stock) y reportarlo para avisar al operador.
