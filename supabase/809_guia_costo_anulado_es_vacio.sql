-- 809_guia_costo_anulado_es_vacio.sql — [DUEÑO] "clickeo un punto morado, o sea donde NO tiene
-- costo, pero al clickear me da un costo. No es lógico: si se anuló es porque no se considera
-- adecuado, así que debería decir la verdad, costo = vacío. Lo que sí puede tener es un botón que
-- diga que existieron costos aquí y fueron eliminados, y al clickearlo me dé esa lista… y hasta
-- quién lo eliminó."
--
-- Tenía razón y sus dos tarjetas lo mostraban solas:
--   30-jul → "📦 Ingreso sin costo"  +  "🚫 costo anulado: S/ 13.20 · Jesus"   ← se contradice
--   24-jun → "📦 Ingreso sin costo"  +  "⚠ esta guía no dejó costo"            ← esta es la verdad
-- La tarjeta mostraba SIEMPRE la última fila de costo de la guía, válida o no. Anular es sacar el
-- costo; mientras el número siga a la vista, no está sacado, está tachado.
--
-- QUÉ CAMBIA EN `mos.curva_guia_detalle`:
--   · `costo` pasa a traer SOLO el costo VÁLIDO de esa guía. Si fue anulado, viene null → la
--     tarjeta dice "esta guía no dejó costo", igual que cuando nunca hubo ninguno.
--   · aparece `costosDescartados[]`: lo que existió y se eliminó, con su motivo y cuántas líneas
--     lo escribieron. Va aparte, detrás de un botón: es historia, no el costo de la guía.
--   · aparece `eliminadoPor`: quién y cuándo ejecutó la reversa (la fila COMPRA_REVERSA guarda al
--     usuario que apretó la ✕). Eso es lo que el dueño pidió poder ver.
...
begin
  ...
  if v_sku is not null then
    -- [809] SOLO el costo VÁLIDO. Si esta guía terminó sin costo, acá viene null y la tarjeta
    -- lo dice sin rodeos, en vez de mostrar un número tachado que ya no significa nada.
    ...
    -- [809] lo que existió y se eliminó — aparte, y solo si alguien lo pide
    select coalesce(jsonb_agg(y.obj order by y.valor desc), '[]'::jsonb) into v_desc
      from (
        select h.valor,
          ...
          from mos.historial_precio_costo h
         where h.tipo='COSTO' and h.sku_base = v_sku and h.id_guia = v_guia
           -- [809] la fila de REVERSA es el ACTO de borrar, no un costo que existió: no va en
           -- esta lista (ya se muestra como "eliminado por X"). Acá solo lo que sí se aplicó.
    ...
    -- [809] quién apretó la ✕: la fila de reversa guarda al usuario que la ejecutó
    select jsonb_build_object(
