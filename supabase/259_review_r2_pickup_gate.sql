-- 259 · FIX Ronda 2 [6 LOW·SUPA] — gate de app en wh.crear_pickup_cierre_caja (escritor sin gate).
-- Cuerpo idéntico a 253 + gate: solo service_role ('') o apps del ecosistema pueden crear el pickup de cierre.
...
begin
  -- [R2 fix] gate de app (antes: solo grant authenticated, cualquier token podía crear pickups).
...
revoke all on function wh.crear_pickup_cierre_caja(jsonb) from public, anon;
