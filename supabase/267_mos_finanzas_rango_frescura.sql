-- 267_mos_finanzas_rango_frescura.sql
-- finanzas_rango NUNCA devolvía _fresh → el front (_getFinanzasRangoDirecto) lo veía undefined y
-- SIEMPRE caía a GAS (consumiendo urlfetch). Con GAS al límite de cuota diaria, esa vista se rompía.
-- Fix: mergear mos._frescura_sombra() en el return (idéntico a finanzas_dia/cierres_caja) → va DIRECTO
-- a Supabase cuando la sombra está fresca, sin pegarle a GAS. Money-safe: si la sombra se congela
-- (_fresh=false) el front cae a GAS igual (gate intacto).
