...
## ✅ Validaciones SUNAT (server-side en fac.emitir_cpe = paridad ME/MOS/convert) + zona VIP
Aplicado + probado (13 emit + 6 convert varieties, 300x CLEAN):
- **FACTURA:** RUC 11 + razón social + dirección (siempre).
- **BOLETA > S/700:** identificar cliente (DNI 8 / RUC 11 / CE-pasaporte) + nombre. ≤700 permite VARIOS.
- **Bancarización ≥ S/2000:** exige medio de pago (Ley 28194); va en `observaciones` del CPE. Frontends
  avisan (MOS panel: selector de medio; ME/MOS: aviso si efectivo ≥2000). **Confirmar umbrales con el contador.**
- **Convert NV→CPE:** nombre editable (cambiar antes/durante); boleta ≤700 VARIOS, >700 exige ID, acepta RUC.
- **Zona VIP (MOS):** el panel Emitir de MOS emite bajo `zona='MOS-VIP'` → su propia serie/correlativo.
  ⚠️ **ACCIÓN go-live:** agregar en `mos.series_documentales` las filas de `id_zona='MOS-VIP'` con su serie
  (BOLETA + FACTURA). Hoy no existe → cae al default `fac.config` (B001/F001). Ver `fac.series_por_zona()`.
...
2. **Definir la serie + correlativo REAL de CADA zona** (ZONA-01, ZONA-02, **y MOS-VIP**) en
   `mos.series_documentales` — cada zona su propia serie (hoy ZONA-01/02 comparten BBB1/FFF1 = demo; MOS-VIP
   aún no existe). Las estaciones de cada zona heredan el correlativo vía la serie (contador único en
   fac.series). Verlo con `fac.series_por_zona()`.
