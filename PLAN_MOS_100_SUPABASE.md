...
- [~] **5. Editor de adhesivos** (`EditorAdhesivos.abrir backendUrl`, index.html:19367) → backend Supabase/Edge. Audit: `AdhesivosPersonalizados.gs` (880 líneas) = CRUD plantillas + TSPL2 gen + PrintNode. 3 stages.
  ...
  - [ ] **5-S2. Edge de impresión (byte-crítico):** portar `_adhJson2tspl` (TSPL2 gen) + iconos + `_adhEnviarPrintNode` a un Edge. Reusa PrintNode de los otros Edges. **Necesita seed de `mos.adhesivo_iconos`** (los hex de ICONOS_BITMAPS_ADH) — backfill desde la hoja o desde el front. Revisión byte-level obligatoria (un byte malo = etiqueta mal impresa).
  - [ ] **5-S3. Cablear el front** (`EditorAdhesivos.abrir backendUrl` + listar/guardar/eliminar/imprimir) a las RPCs/Edge, gateado (`mos_adhesivos_edge`, default OFF) + GAS de red de seguridad. SW bump + deploy + backfill de plantillas existentes desde la hoja.
