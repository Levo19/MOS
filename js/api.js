...
const API = (() => {
  ...
  async function _postDirectoMOS(action, params) {
    ...
    // [v2.43.611] Deshacer RETROACTIVO del costo que una compra aplicó al catálogo (SQL 556):
    // revierte precio_costo del canónico al costoAnterior de esa guía + limpia su historial COMPRA.
    if (action === 'quitarCostoCompra') {
      const r = await _sbRpcMOS('quitar_costo_compra', { p: {
        idGuia: p.idGuia, usuario: _mosUsuario(p), items: p.items
      } }, 'mos');
      if (r == null) return null;
    ...
    // [819] Desglose de un SKU del día: cuánto se vendió suelto y cuánto en cada presentación,
    // con el MISMO costo por unidad base. Mismo universo que finanzas_dia (sin crédito). PURA.
    if (action === 'finanzasDiaSku') {
      const r = await _sbRpcMOS('finanzas_dia_sku', { p: { fecha: p.fecha || '', skuBase: p.skuBase } }, 'mos');
      if (r == null) return null;
      return r;   // {ok, data:{skuBase, costoUnitBase, margenDefault, items[]}}
    }
    if (action === 'cabinaSemanal') {   // [948] motor del dashboard "Cabina Semanal" (semana en curso o pasada por offset)
      const r = await _sbRpcMOS('cabina_semanal', { p: { offset: (p && p.offset) || 0 } }, 'mos');
      if (r == null) return null;
    ...
    if (action === 'cabinaHeatDias') {   // [953] mapa de calor por día (lun→dom), hora×zona
      const r = await _sbRpcMOS('cabina_heat_dias', { p: { offset: (p && p.offset) || 0 } }, 'mos');
      if (r == null) return null;
      return r;   // {ok, data:{semana, maxVenta, dias:[{fecha,dow,futuro,total,tickets,heat}]}}
    }
    if (action === 'cabinaPersonal') {   // [952] planilla semanal de personal (por zona/almacén, día por día)
      const r = await _sbRpcMOS('cabina_personal', { p: { offset: (p && p.offset) || 0 } }, 'mos');
      if (r == null) return null;
      return r;   // {ok, data:{semana, grupos:[{zona,subtotal,personas:[{...,porDia}]}], totales}}
    }
    if (action === 'cabinaReposicion') {   // [950] reposición & considerados: fill-rate real (SALIDA vs creado)
      ...
      if (r == null) return null;
      return r;   // {ok, data:{fillRatePct, total, despachados, pendientes, vencidos, horasMediaDespacho, porZona, items}}
    }
    if (action === 'cabinaDrill') {   // [949] detalle de una barra/celda del dashboard (dia|hora|zona)
      const r = await _sbRpcMOS('cabina_drill', { p: {
        tipo: (p && p.tipo) || '', zona: (p && p.zona) || '',
        hora: (p && (p.hora != null) ? p.hora : ''), fecha: (p && p.fecha) || '',
        offset: (p && p.offset) || 0
      } }, 'mos');
      if (r == null) return null;
      return r;   // {ok, data:{comisiones|quienes|estancados}}
    }
    // [820] Desglose POR TRAMO de precio: reconstruye qué tramo aplicó cada venta con la misma
    // regla del POS (gramos + bordes inclusivos). Responde cuál tramo deja más margen. PURA.
    if (action === 'finanzasDiaSkuTramos') {
      const r = await _sbRpcMOS('finanzas_dia_sku_tramos', { p: { fecha: p.fecha || '', skuBase: p.skuBase } }, 'mos');
      if (r == null) return null;
      return r;
    }
    ...
    if (action === 'guardEstado') {   // [881] MosGuard: equipos con su ubicación + estado
      const r = await _sbRpcMOS('yape_guard_estado', { p: {} }, 'mos');
      if (r == null) return null;
      return r;
    }
    if (action === 'guardMarcar') {   // [881] marcar un equipo ROBADO / NORMAL
    ...
    if (action === 'buzonSubir') {    // [885] IGV a favor · buzón: OCR de una factura de compra
      try {
        const tk = await _mintTokenMOS();
        const res = await fetch(`${_SB_URL}/functions/v1/buzon-igv`, { method: 'POST',
          headers: { 'apikey': _SB_ANON, 'Authorization': 'Bearer ' + tk, 'Content-Type': 'application/json' },
    ...
    if (action === 'buzonListar') {   // [885] listar el buzón del mes
      const r = await _sbRpcMOS('igv_buzon_listar', { p: { mes: p.mes, anio: p.anio } }, 'mos');
      if (r == null) return null; return r;
    }
    if (action === 'buzonBorrar') {   // [885] borrar una entrada del buzón
      const r = await _sbRpcMOS('igv_buzon_borrar', { p: { idBuzon: p.idBuzon } }, 'mos');
      return r && r.ok !== false ? { status: 'success' } : { status: 'error', error: (r && r.error) || 'no se pudo' };
    }
    ...
    if (action === 'yapeAtarGlobal') {   // [959] 1 Yape = suma de varios tickets
      const r = await _sbRpcMOS('yape_atar_global', { p: {
        id: p.id, idVentas: p.idVentas || [], usuario: p.usuario || '' } }, 'mos');
      if (r == null) return null;
      return r;
    }
    ...
    if (action === 'iaPendientes') {   // [937] cola de pendientes de IA en tiempo real (gestión/reconciliación)
      const r = await _sbRpcMOS('ia_pendientes_listar', { p: {} }, 'mos');
      if (r == null) return null;
      return r;
    }
  ...
  const _MOS_POST_DIRECTO = {
    ...
    cabinaPersonal:              () => true,   // [952] planilla semanal
    cabinaHeatDias:              () => true,   // [953] mapa calor por dia
    ...
    buzonSubir:                  () => true,   // [885] IGV a favor: subir factura al buzón
    buzonListar:                 () => true,   // [885] IGV a favor: listar buzón
    buzonBorrar:                 () => true,   // [885] IGV a favor: borrar del buzón
  ...
  return {
    ...
    zona: {
      ...
      // [790] ingresos de mercadería (proveedor/envasado) por sku de los últimos N días —
      // alimenta el matiz PRIORIZADO ("🆕 ingresó") del semáforo del pickup.
      ...
      // [1022] Catálogo completo estructurado (base + presentaciones/derivados/equivalencias/tramos/historial)
      //   para la vista háptica interactiva y el exportador a Excel (una hoja jerárquica).
      catalogoExport: async () => _sbRpcMOS('catalogo_export', { p: {} }, 'mos'),
