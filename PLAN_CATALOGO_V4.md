...
V5 EN DISEÑO (dibujado §10-§11 del artifact, PENDIENTE OK del dueño para codear): PRECIO·COSTO·MARGEN — click al precio del card → modal curvas (precio=historial_cambios ya existe · costo=historizar precioCosto NUEVO + compras) + bidireccional precio↔margen (margen sobre precio, pv=costo/(1−m), redondeo 1 DECIMAL) que al guardar fija el margen como CONTRATO del producto (override de política); margen POR NIVEL independiente (granel margen1, 500GR margen2); costos de satélites SIEMPRE derivados del padre × factor (fcb derivado, factor pack — NO editables, NO duplicados en BD: derivar en render/RPC); GUARD: sin costo registrado → SIN margen (no calcular/propagar nada). SECUENCIAL DE COMPRA (reemplaza overlay costos+OCR+ticket jefa): guías proveedor con badge 🧾COMPRA + filtro + estado "costos pendientes"; Paso1 montos(Total/Unit·IGV)→costos propagados ×factor + HISTORIAL DE COSTOS(fecha/guía/usuario); Paso2 (jefa en tablet) cascada con márgenes respetados AUTO/MANUAL/excluir + producto-sin-costo-previo="DEFINIR a mano, nace contrato" + volver a Paso1; ELIMINAR: OCR completo de guías (chips/re-OCR/badges 🤖) + guardarCostosEImprimirJefa/ticket. Flujo actual inventariado: overlay costos app.js ~8140-8260 (OCR chips, modos TOTAL/UNIT, IGV, updMaster), sugerencias _sugerencia._precioEditado ~11350-11660, guardarCostosGuia 11597, botón imprimir jefa 8032.
RONDA 6 ✅ (v2.43.494): ① copy rollo ("ROLLO físico: etiquetas usadas desde calibración/capacidad") ② stepper adhesivo sugiere por ROTACIÓN semanal del derivado (adhSugerido desde cache; último-usado > rotación > 12) + subtítulo "sugerido ~X/sem" ③ CESTA PURGA con HIJOS jerárquicos: derivados+packs listados bajo el granel (filas 🥄 violeta, claves PRE|id), marcar CAN auto-marca hijos (toast "incluye N hijos"), contador Np·N🥄·Ne, ventasTotal incluye hijos — coherente con guard 429 ④ modal tramo: unidad g/kg (_satTramoG) + brackets [ ( ) ] alternables (_satBracket, minIncl/maxIncl reales) visibles en escalera y lista ⑤ HISTORIAL verificado en BD: precios SÍ (historial_cambios: precioVenta/motivo/usuario/ts, 164 productos con entradas — chart del panel lo lee); COSTOS NO se historizan (0 entradas; viven por compra en pedidos_proveedor → sección Historial de compras del panel). Futuro posible: historizar precioCosto en actualizar_producto.
RONDA 5 ✅ (v2.43.491-493) — AUDITORÍA RETROSPECTIVA COMPLETA (pedido del dueño "no puedo revisar como humano"): matriz 60 promesas → 52 cumplidas + 8 gaps TODOS cerrados: ① stepper adhesivo (IIFE mal escapada = mi fix R4 roto; ahora memoria por producto localStorage mos_adh_qty_<cb>, default 12) ② chips de alcance fusionada (ALL/CANON/derivados/PACKS → _agAlcance con codigos) ③ stat valor despachado ≈ (precio canónico × cant) ④ badge 🥄 N derivados ⑤ toggle+print en packs de derivado ⑥ toast solape tramos ⑦ toggle membretes OFF por apertura = DESVIACIÓN DELIBERADA (opt-in seguro) ⑧ código PRE-generado al abrir creación + botón "Regenerar". PURGA directriz #2: ~600 líneas muertas fuera (editor tramos embebido seg* completo + #modalSegmento + #modalSegmentosSection, equivalencias embebidas + #modalEquivSection + 5 fns, toggleDerivs, íconos gondola/andamio/rollo huérfanos, exports) — conservados _segColorMap/_segRegiones/_segLadderCells/_segLadderAxis/_segFmtPeso (escalera del card) y _loadEquivModal (refresco equivMap). FIX 493: div sin cerrar del skeleton anidaba anContent en anLoading (hallazgo auditor UI). Equivalencias/tramos: gestión 100% en modales ＋. UI audit 8/8 pantallas verificadas (outputs en browsercheck/_audit_out_*.txt) + smoke final 493 todo verde (chips alcance 3, stepper 12 sano, 1538 cards post-purga). PENDIENTE de decisión del dueño: borrar device de prueba 7e57c1a0 + prueba física impresora/cámara.
RONDA 4 ✅ (v2.43.490): ① ANALÍTICA FUSIONADA VIVA — causa raíz del "sin tabs" (dueño Y harness): temp table + pool PostgREST → plan cacheado con OID muerto → HTTP 400 silencioso. SQL 430 reescribe mos.analitica_grupo SIN temp table (grupo como CTE _g_raw/_ag_grupo con distinct on prio, función stable). Verificado por el camino REAL: tabs 🏭 Almacén/🏪 ZONA-02/🏪 ZONA-01 ESTIMADA + kg/sem (shot_s9). Lección: RPC nueva con temp table + PostgREST = bomba; usar CTE. ② stepper adhesivo: recuerda última cantidad POR PRODUCTO (localStorage mos_adh_qty_<cb>), default 12 (el 24 era arbitrario). ③ escalera de tramos del card → SIEMPRE abre modal 📊 Tramo nuevo (cells + "+ tramo" + ✏). ④ edición sin barra de tipos: título "Editar · ⚗️ Granel envasable" etc. NOTA harness: browsercheck primera corrida recarga por SW (waitMs 26000 evita "context destroyed").
RONDA 3 ✅ (v2.43.489): EDITAR POR TIPO — abrirEditarProducto rutea: derivado→modal 🥄 prellenado (Guardar cambios vía actualizarProducto, validación ignora propio id), presentación→modal 🧱, canónico→modal completo; link "⚙️ edición avanzada" al completo. Verificado en harness (shot_s7). INTEGRIDAD JERÁRQUICA SQL 429 (smoke 3/3): no borrar padre con derivados vivos (statement-level: purga padre+hijos JUNTOS sí pasa), no cambiar unidad de granel con derivados; bypass GUC mos.skip_cb_guard.
