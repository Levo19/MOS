...
- 2026-07-18 (PENDIENTES CERRADOS, MOS 2.43.575 · WH 2.13.446 · ME 2.8.207): (1) PRESENCIA ecosistema COMPLETA: WH y ME se anuncian al canal 'ecos-presencia' (mismo patrón sobre su WS de catálogo; teardown en logout; scripts inline validados node --check 11+7 OK) → Config ve cajas/tablets EN LÍNEA AL SEGUNDO. (2) FLIP WH_REGISTRAR_PN_DIRECTO=1 (atómico 217/218; verificado con datos: 5 aprobaciones post-cutover intactas 1 semana, sombra 80 filas fresca) + getProductosNuevosWH SIN fallback GAS (transitorio→[]). Browsercheck flujo MOS completo: "✅ CERO fetches a GAS". (3) tarifa_envasado NO era basura: la leía wh.get_tarifa_envasado (423) para el "Tu pago" del modal WH → SQL 515: lee la canónica evalTarifaEnvasadoPorUnidad con fallback legacy (fix drift display vs pago real). NOTA: WH Pages vive en /warehouseMos-/ (repo con guión).
