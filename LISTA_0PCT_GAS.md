...
## SESIÓN 2026-07-08 noche (parte 2, Fable 5) — F3/F6 completos + verificados
- ✅ **F3 COMPLETO** (MOS 2.43.474/475): `ocrComprobanteGuia` + `ocrTicketJefa` → Edge `ia` (Vision, prompts portados). **OCR verificado END-TO-END con foto real** (get_reporte→foto→Vision→items+confidence). `imprimirCostosGuia` → ESC/POS client + Edge imprimir. + fix bug get_reporte (monto numeric→text, cazado por el test).
- ✅ **F6 COMPLETO** (MOS 2.43.476): `getSugerenciaPrecioIndividual` → mos.sugerencia_precio_individual (SQL 411, precio catálogo + sugerido por margen). `wh_editarPNCantidad` → wh.editar_pn_cantidad (SQL 412, orquesta PN + `_sync_lote_desde_detalle` [ya portado] + guia_detalle, guard PENDIENTE, atómico). Verificado con RPC. **Hotfix**: un `await` en post() no-async rompió api.js 1 min → corregido (async IIFE) + MOS verificado sano en vivo.
