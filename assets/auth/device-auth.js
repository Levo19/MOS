...
//   │ previa EXITOSA de ESTE MISMO deviceId con ≤24h, la app OPERA en modo
...
//   │ o (c) la verificación previa supera las 24h. Un HTTP 4xx (el server habló)
...
//   │ módulo ya documentaba; se acota a 24h, se BORRA en _invalidarCache ante
...
(function() {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  var _GRACIA_MS = 24 * 60 * 60 * 1000;             // umbral conservador: 24h
  ...
  // ¿Hay una verificación previa VÁLIDA (mismo deviceId) y RECIENTE (≤24h)?
  function _graciaElegible() {
    try {
      ...
      // Bootstrap: equipos verificados ANTES de v1.0.30 no tienen la marca ms. Se
      // acepta sólo si la fecha Lima cacheada es HOY (≤24h por construcción) — y
      // _invalidarCache la borra ante cualquier bloqueo, así que no rescata a nadie.
      var d = _lsGet(_config.storageKeys.lastVerifyDate);
      return !!d && d === _fechaHoyLima();
    } catch (_) { return false; }
  }
  ...
  function _entrarEnGracia(motivo) {
    ...
    try { console.warn('[DeviceAuth] GRACIA (' + motivo + '): verificación previa válida ≤24h y el servidor no responde → la app opera mientras se reverifica en background.'); } catch (_) {}
  ...
  function _cerrarPorGraciaVencida(e) {
    ...
    _mostrarUI('SIN_VERIFICAR', (e && e.message) || 'No se pudo reverificar el dispositivo. Revisa tu conexión.');
    if (_config.onError) { try { _config.onError(e || new Error('gracia vencida')); } catch (_) {} }
  }
  ...
  function _armarWatchdog() {
    ...
    _state.watchdogTimer = setTimeout(function() {
      ...
      // [v1.0.30] El cuelgue es un fallo de RED (el server nunca contestó). Si este
      // equipo tiene verificación previa válida ≤24h → GRACIA en vez de portazo.
      if (_graciaElegible()) { _entrarEnGracia('watchdog ' + _WATCHDOG_MS + 'ms'); return; }
  ...
  function _failClosedVerify(silencioso, e) {
    ...
    // equipo tiene una verificación previa válida ≤24h → operar en modo
