...
- ✅ **Fase 1 CIMIENTOS (SQL 100, commits 3b13ba7+2ae689b, INERTE)**: 2 columnas (fecha_caducidad/desbloqueo_temporal_hasta) + `mos._validar_clave_admin_core` (bcrypt sin gate, wrapper conserva el gateado) + 4 RPCs anon-callable (registrar/verificar/aprobar/revocar, aprobar/revocar con clave admin core + master-only para MOS) + `mos.auth_intentos` (lockout exponencial cap 2^16, fix overflow aplicado) + get_flags con `dispositivos_revocados[]`+`device_verify_version`. **Revisión adversarial 40x: SEGURO** (sin bypass clave, core protegida de anon, lockout antes del bcrypt, auditoría completa). Diseño completo: `DISENO_auth_dispositivos_cutover_supabase.md`.
- 🔴🔵 **PRÓXIMA SESIÓN DEDICADA — Fase 2 SYNC INVERSO** (decisión del usuario 2026-06-16: no apurar, es seguridad crítica). RETOMA: (1) invertir sync `mos.dispositivos→HOJA` y APAGAR el viejo HOJA→sombra (no coexisten, repetiría pisado); (2) migrar ~40 lectores de la hoja DISPOSITIVOS en las 3 apps (mint cross-app WH/ME, paneles, bloqueo por usuario, espía/audio que joinean por Ultima_Sesion) uno por uno; (3) poblar ultima_sesion/ultima_zona en la sombra. Luego Fase 3 (cablear device-auth.js a las RPCs detrás de flag DEVICE_AUTH_DIRECTO + cache-first + denylist + **UX moderno con efectos** — MOSTRAR mockups al usuario) y Fase 4 (pg_cron: purga 7d + reversión desbloqueos + cancelar pendientes; alertas a seguridad_alertas; push aprobación). Riesgo residual a cerrar en cutover: rate-limit por IP (hoy solo por-device).
- ~~Core auth 100% Supabase (en diseño)~~ → ver Fase 1 ✅ + Fase 2 arriba.
- 🔵 **(legacy plan, integrado arriba)**: RPCs directas registrar/verificar/aprobar/revocar → `mos.dispositivos` (anon-callable seguro como mint-mos; aprobar reusa `verificar_clave_admin` bcrypt ya existente). Retira la hoja DISPOSITIVOS del camino de auth. mint-mos ya lee la sombra.
